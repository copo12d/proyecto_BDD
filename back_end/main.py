...
def main():
    ...
    db = Database("127.0.0.1","root","30412187","pruebacascada")
    marca = Marca(db)
    pieza = Piezas(db)
    modelo = Modelo(db)
        ...
        #---------DEFINIENDO LA RAIZ Y LA IMAGEN DEL INICIO---------
            ...
            def widgetstablas(frame):
                ...
                if frame==modeloframe:
                    ...
                    Modelo_idModeloentry=Entry(vehiculoframe, width=30)
                    Añoentry=Entry(modeloframe, width=30)
                    idVehiculoentry.grid(row=0,column=1)
                    Tipo_Vehiculoentry.grid(row=1,column=1)
                    Modelo_idModeloentry.grid(row=2,column=1)
                    Añoentry.grid(row=3,column=1)
                    
                    
                    def agregarbtn():
                        
                        vehiculo.insert(int(idVehiculoentry.get()),Tipo_Vehiculoentry.get(),int(Modelo_idModeloentry.get()))
                        idVehiculoentry.delete(0,END)
                        Tipo_Vehiculoentry.delete(0,END)
                        Modelo_idModeloentry.delete(0,END)
                        tablac()
                    ...
                    def editarbtn():
                        ...
                        Modelo_idModelovalor=tabla.item(tabla.selection())['values'][2]
                        
                        
                        idVehiculoentry.insert(0, idvehiculovalor)
                        Tipo_Vehiculoentry.insert(0, Tipo_Vehiculovalor)
                        Modelo_idModeloentry.insert(0, Modelo_idModelovalor)
                    ...
                    def borrarbtn():
                        global tabla
                        id_vehiculo=tabla.item(tabla.selection())['values'][0]
