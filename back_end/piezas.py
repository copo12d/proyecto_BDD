...
class Piezas:
    ...
    def generate_pdf(self, nombre):
        sql = """SELECT mo.Nombre_Modelo, co.Nombre_Componente, pi.nombre_pieza, ve.año
                FROM modelo mo
                JOIN marca ma ON mo.Marca_idMarca = ma.idMarca
        ...
        elements.append(Paragraph("Registro de Piezas con Año", style_heading))

        data = [["Nombre del modelo", "Nombre del componente", "Nombre de la pieza", "Año"]]
        for registro in resultados:
            nombre_modelo = str(registro[0])
            nombre_componente = str(registro[1])
            nombre_pieza = str(registro[2])
            año = str(registro[3])
            data.append([nombre_modelo, nombre_componente, nombre_pieza, año])

        # Creamos la tabla
        tabla = Table(data, colWidths=[150, 150, 150, 50])
